...
def test_browser_rate_overrides_leave_auth_defaults_unchanged(tmp_path, monkeypatch):
    ...
    expected["auth"].update(rate_window_seconds=1, login_limit=100)
    ...
    assert application.state.auth.google.config == application.state.auth.config

    assert config.auth.rate_window_seconds == 60
    assert config.auth.login_limit == 10
    assert load_config() == config
    assert auth_app(config, environment).state.auth.config == config.auth
