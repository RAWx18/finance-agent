...
export const VoiceOrb: FC<VoiceOrbProps> = memo(
  ({ state = "idle", volume = 0, variant = "default", className }) => {
    ...
    const render = useCallback(() => {
      ...
      const s = reducedMotion.current ? 1 : 0.045;
      ...
      const elapsed = reducedMotion.current ? 0 : (performance.now() - startTime.current) / 1000;
      ...
      const vol = reducedMotion.current ? 0 : volumeRef.current;
      ...
      if (!reducedMotion.current) animRef.current = requestAnimationFrame(render);
    }, [colors]);
