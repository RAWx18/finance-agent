...
export function PlanningPossibilities({ snapshot }: { snapshot: Snapshot }) {
  ...
  return <section className="planning-possibilities" aria-label="Planning possibilities">
    {impact && <div className="planning-allowance">
      ...
      <ul className="planning-items">{impact.items.slice(0, 2).map(item => <li key={item.recordId}><strong>{item.label}</strong> · {cardMoney(item.amountPaise)}{item.status === 'estimate' && ' · Estimated amount'}{item.amountBasis === 'requiredOnly' && ' · Minimum only; intended total unknown'}</li>)}</ul>
