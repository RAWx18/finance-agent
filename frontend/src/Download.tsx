...
export function Download({ label, primary = false }: { label: string; primary?: boolean }) {
  const pending = useRef(false);
  const [busy, setBusy] = useState(false);
  return <a className={`button${primary ? ' primary' : ''}`} href="/api/session/export" download aria-disabled={busy} onClick={async event => {
  ...
  }}>{busy ? 'Preparing download…' : label}</a>;
}