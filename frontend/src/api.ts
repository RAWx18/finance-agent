...
export const api = {
  ...
  startCall: () => request<CallJoin>('session/call', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}',
  }),
  endCall: () => request<CallState>('session/call', { method: 'DELETE', keepalive: true }),
...
export function errorMessage(error: unknown, operation?: Command['operation']['type']): string {
  ...
  if (error.body.code === 'stalePreview') {
    ...
    if (operation === 'discardPreview') return 'This preview is no longer available to reject. Review the current proposal before trying again.';
