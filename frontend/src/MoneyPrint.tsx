...
export function MoneyPrint({ snapshot }: { snapshot: Snapshot }) {
  ...
  return <article hidden className="money-print print-only" aria-label="Saved plan for printing">
    ...
    <h2>Reported items</h2><ul>{snapshot.facts.records.map(item => <li key={item.id}>{item.label}: {item.schedule.amounts?.length ? 'Varies by occurrence' : <>{amountLabel(item.amount)} · {factStatus(snapshot, 'amount', item)}</>}
      ...
      {' · '}{item.schedule.date ? dateLabel(item.schedule.date) : 'Date unknown'} · {factStatus(snapshot, 'schedule.date', item)} · {scheduleLabel(item.schedule)}
      {item.schedule.recurrence === 'monthlyBudget' && <p>Amount per calendar month. {budgetDescription}</p>}
      <MoneySources record={item} snapshot={snapshot} /></li>)}</ul>
