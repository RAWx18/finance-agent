...
test('120 dated events paginate and search the complete backend timeline without growing the document', async ({ page }, info) => {
  ...
  await expect(page.getByText('120 items', { exact: true })).toHaveClass('sr-only');
  ...
  await expect(filter.getByRole('option')).toHaveText(['All items', 'Money in', 'Money out']);
  await page.getByRole('searchbox', { name: 'Search upcoming items', exact: true }).fill('');
  await filter.selectOption('income'); await expect(page.getByText('40 items found', { exact: true })).toBeVisible();
...
test('custom change selection reads all eligible backend occurrences without saving or clipping its dialog', async ({ page }, info) => {
  ...
  const add = page.getByRole('button', { name: 'Choose a payment', exact: true }); await add.click();
