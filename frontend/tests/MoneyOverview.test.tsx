...
it('shows server totals with the first and largest shortfalls, not a misleading closing surplus', async () => {
  ...
  expect(metrics).toHaveTextContent('Opening cash₹5,00011 Sept');
  expect(metrics).toHaveTextContent('Expected income included₹30,000');
  ...
  expect(within(attention).getByRole('heading')).toHaveTextContent(financialText(saved.plan.decisionAssessment!.outcome!.headline));
  expect(within(attention).getByText(saved.plan.decisionAssessment!.outcome!.summary)).not.toBeVisible();
...
it.each([
  ...
  saved.plan = { ...saved.plan, projectionPartial: false, reliableIncomePaise: 500000, outflowPaise: 600000,
    planningFacts: structuredClone(saved.facts),
...
it('shows formatted qualifications once in calculation details rather than repeating them in the overview', async () => {
  const saved = planningSnapshot();
  ...
  expect(closing).not.toHaveTextContent('Uses estimated Rent');
  expect(closing).toHaveTextContent('Includes estimates · Not a spending allowance');
  for (const qualification of screen.queryAllByText('Uses estimated Rent (₹12,000) on 13 Sept 2026.')) expect(qualification).not.toBeVisible();
  const attention = screen.getByRole('region', { name: 'What needs attention' });
  ...
  for (const action of within(attention).getAllByText('Contact the provider before the due date.')) expect(action).not.toBeVisible();
  await userEvent.click(within(attention).getByRole('button', { name: 'Discuss payment options' }));
  const action = screen.getByRole('dialog', { name: 'Your next step' });
  expect(within(action).getByText('Contact the provider before the due date.')).toBeVisible();
  await userEvent.click(within(action).getByRole('button', { name: 'Close your next step' }));
  await userEvent.click(screen.getByRole('button', { name: 'View calculation' }));
  const details = screen.getByRole('dialog', { name: 'Plan details' });
  expect(within(details).getByText('Uses estimated Rent (₹12,000) on 13 Sept 2026.')).toBeVisible();
...
it.each([
  { recurrence: 'monthlyBudget', label: 'Daily share · Monthly budget', count: 28, interval: 1, nextDate: '14 Sept' },
  { recurrence: 'weekly', label: 'Weekly budget', count: 4, interval: 7, nextDate: '20 Sept' },
] as const)('groups $recurrence occurrences using current dates without combining records or recomputing amounts', ({ recurrence, label, count, interval, nextDate }) => {
  const saved = planningSnapshot();
  saved.plan.evaluatedOn = '2026-09-14';
  saved.plan.events = Array.from({ length: count }, (_, index) => {
    const date = new Date(Date.UTC(2026, 8, 13 + index * interval)).toISOString().slice(0, 10);
    return { ...saved.plan.events[0], id: `budget:${index}`, recordId: 'budget', label: 'Living budget', date, originalDueDate: date, amountBasis: 'budget' as const, amountPaise: 10000 };
  });
  saved.plan.events.splice(2, 0, { ...saved.plan.events[1], id: 'other', recordId: 'other', label: 'Living budget', amountPaise: 12345 });
  saved.facts.records = [{ ...saved.facts.records[0], id: 'budget', label: 'Living budget', schedule: { date: '2026-09-13', recurrence, certainty: 'exact', basis: 'payment' } },
  ...
  expect(rows[0]).toHaveTextContent(`${nextDate} · ${label}`); expect(rows[0]).toHaveTextContent('−₹100');
  expect(rows[0]).not.toHaveTextContent('13 Sept');
...
it('keeps a reserve breach distinct from missing details and cash shortage', () => {
  ...
  saved.plan.decisionAssessment!.outcome = { ...saved.plan.decisionAssessment!.outcome!, branch: 'uncertain', headline: 'Your cash buffer is at risk.', summary: 'Dated payments fit, but the cash buffer is not protected.' };
